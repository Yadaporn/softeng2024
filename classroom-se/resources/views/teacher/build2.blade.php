...
@section('content') 
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $sql = 'SELECT COUNT(*) FROM status_build2 WHERE st_room = ? AND st_day = ? AND ((st_start < ? AND st_end > ?) OR (st_start < ? AND st_end > ?))';
        ...
        try {
            ...
            $stmt2 = $pdo->query("SELECT crs_room, crs_Number_of_students FROM tb_build WHERE crs_build = '2'");
        ...
        ?>
        <!-- Display the data -->
        <form action="{{ route('classroom.adddd') }}" method="post">
            @csrf
                ...
                <div id="errorModal" class="modal" style="display: block;">
                    <div class="modal-content">
                        ...
                        <div class="button-container">
                            ...
                            <button type="button"onclick="window.location.href='statusbuild2';" class="status-btn">สถานะ</button>
                        </div>
                    </div>
                </div>
            @endif
