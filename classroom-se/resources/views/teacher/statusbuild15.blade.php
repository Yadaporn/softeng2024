...
<body>
    ...
    $sql = 'SELECT * FROM status_build15';
    ...
    $total_items_query = 'SELECT COUNT(st_id) FROM status_build15 WHERE st_room LIKE ? OR st_day LIKE ? OR st_start LIKE ? OR st_end LIKE ?';
    ...
    $sql = 'SELECT * FROM status_build15 WHERE st_room LIKE ? OR st_day LIKE ? OR st_start LIKE ? OR st_end LIKE ? LIMIT ? OFFSET ?';
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('ssssii', $searchTermLike, $searchTermLike, $searchTermLike, $searchTermLike, $items_per_page, $offset);
    $stmt->execute();
    ...
    echo '<table border="1">';
    echo '<tr><th>ห้อง</th><th>วัน/เวลา</th><th>สถานะ</th></tr>';
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<td>';
            echo '<button onclick="window.location.href=\'' . route('statusbuild15name') . '\'" class="next-page-button" style="border: none; background: none; padding: 0; outline: none;">';
            echo '<img src="asset/cross.png" alt="nextpage" id="nextpage"style="width: 40px; height: 40px;">';
            echo '</button>';
            echo '</td>';
