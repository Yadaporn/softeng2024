...
<body>
    ...
    $total_items_query = 'SELECT COUNT(st_id) FROM status_build2 WHERE st_room LIKE ? OR st_day LIKE ? OR st_start LIKE ? OR st_end LIKE ?';
    ...
    $sql = 'SELECT * FROM status_build2 WHERE st_room LIKE ? OR st_day LIKE ? OR st_start LIKE ? OR st_end LIKE ? LIMIT ? OFFSET ?';
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<td>';
            echo '<button onclick="window.location.href=\'' . route('statusbuild2name') . '\'" class="next-page-button" style="border: none; background: none; padding: 0; outline: none;">';
